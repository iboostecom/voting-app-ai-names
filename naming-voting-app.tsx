      ...
      {getAllVoters().length > 1 && (
        <div className="mt-8 bg-white rounded-xl shadow-lg p-6">
          ...
          <div className="mt-6 text-center">
            <button 
              onClick={() => {
                const allVotes = getAllVoters().map(voter => {
                  const names = getVoterSelectedNames(voter).map(item => item.name);
                  return `${voter}: ${names.join(', ')}`;
                }).join('\n');
                navigator.clipboard.writeText(allVotes);
